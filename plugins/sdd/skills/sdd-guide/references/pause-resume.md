...
- A pause during `/sdd:discovery` writes `docs/discovery-resume.md`.
- A pause during `/sdd:refine` writes `docs/refine-resume.md`.
- A pause during `/sdd:validate` writes `docs/validate-resume.md`.
...
| Section | Quality bar |
|---|---|
| **Status** | 1–2 sentences. Names the in-flight command and the user-visible position in its flow. Example: "Paused mid-`/sdd:refine`, marker walk at [GAP g2] with three markers resolved." |
...
| **Instructions for next-session** | Direct address to the resumed agent. Names the resumed command, beat or round number, and the immediate next action. Example: "You are resuming `/sdd:refine` at the marker walk, next marker [GAP g2]. Read the files above, confirm the decisions in this file with the user, then present the pending marker." |
...
- **No re-explanation of plugin behavior.** Point at the source `SKILL.md` or reference file; don't paraphrase it.
  - Anti-pattern: "The `/sdd:refine` command walks markers one at a time where each marker is presented with context..."
  - Corrected: "Resuming `/sdd:refine` — see `plugins/sdd/skills/refine/SKILL.md` and `references/markers.md`."
...
The resumed command is responsible for deleting its own resume file. The delete fires when the underlying interview completes its document-generation step — i.e., once the resumed command has successfully produced or updated its target artifact (`docs/plan.md`, etc.).
